...
Let's plot the coordinates which we have interpolated values for from kriging over the top.
...
You can get the centroid of each polygon by using `sf::st_centroid()` and the coordinates out of this object with `st_coordinates()`. 
